  ...
  , mDefaultFieldRegexp( QStringLiteral( "^(?:field_)(\\d+)$" ) )
{
  ...
  mMaxBufferSize = bufferSizeStr.isEmpty() ? 512 * 1024 * 1024 : bufferSizeStr.toInt();
}
...
qint64 QgsDelimitedTextFile::size()
{
...
bool QgsDelimitedTextFile::setFromUrl( const QUrl &url )
{
  ...
  if ( query.hasQueryItem( QStringLiteral( "encoding" ) ) )
  {
    ...
    mEncodings.append(query.queryItemValue( QStringLiteral( "encoding" )));
  }
...
QUrl QgsDelimitedTextFile::url()
{
  ...
  if (mEncodings.size() > 0 && mEncodings[0] != QLatin1String( "UTF-8" ) )
  {
    query.addQueryItem( QStringLiteral( "encoding" ), mEncodings.at(0) );
  }
...
void QgsDelimitedTextFile::setEncoding( const QStringList &encodings )
{
  resetDefinition();
...
QgsDelimitedTextFile::Status QgsDelimitedTextFile::nextLine( QString &buffer, bool skipBlank )
{
  ...
  if (mLineNumber == 0)
  {
    const QByteArray data = mFile->read(mMaxBufferSize) ;

    QTextCodec::ConverterState state;

    mPosInBuffer = 0;

    if (mEncodings.isEmpty()){

        mCodec = QTextCodec::codecForName(QStringLiteral( "UTF-8" ).toLatin1());

        mBuffer = mCodec->toUnicode(data.constData());

    }
    else{

        foreach(QString codec, mEncodings) {

           mCodec = QTextCodec::codecForName(codec.toLatin1());

           mBuffer = mCodec->toUnicode(data.constData(),data.size(),&state);
